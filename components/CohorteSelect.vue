    ...
    <div class="pr-4">
      <div class="relative z-0 inline-flex rounded-md mt-3 w-full">
        ...
        >
          ...
          <p v-else class="text-left font-medium">
            Localisation : {{ selected.label }}
          </p>
        </div>
        <button
          type="button"
      ...
      >
        <div class="flex flex-col">
          <div class="flex justify-between">
            ...
            <p
              :class="{
                'font-semibold': isSelected(cohorte),
              }"
            >
              {{ cohorte.label }}
            </p>
            <!--
                                      ...
                                      -->
            <span v-if="isSelected(cohorte)" class="text-indigo-500">
          ...
          <p class="text-gray-500 mt-2">Région {{ cohorte.region }}</p>
        </div>
      </li>
    </ul>
  </div>
...
export default defineComponent({
  props: {
    cohortes: {
      type: Array as PropType<Cohorte[]>,
      required: true,
    },
    selected: {
      type: Object as PropType<Cohorte>,
      default: null,
    },
  },
  setup(props, { emit }) {
    ...
    const select = (diplome: Cohorte) => {
      toggle()
      emit('select', diplome)
    }

    const isSelected = (diplome: Cohorte) =>
