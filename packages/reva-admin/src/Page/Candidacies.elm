...
withStatusFilter context status model =
    ...
    in
    ( { model | filters = model.filters |> withNewStatus }, Api.Candidacy.getCandidacies context.endpoint context.token GotCandidaciesResponse (toStatusFilterEnum status) )
