...
test.describe("CertificationAuthorityContactTile", () => {
  ...
  test("should display contact information when available", async ({
    ...
    await expect(certificationAuthorityContactTile).toContainText("John Doe");
    await expect(certificationAuthorityContactTile).toContainText(
      "john.doe@authority.test",
    );
  });

  test("should display local accounts contact information when available", async ({
    page,
    msw,
  }) => {
    const candidacy = createDashboardCandidacy({
    ...
    await expect(certificationAuthorityContactTile).toContainText("0123456789");
  });
