...
const PAGE_URL = `candidates/${candidate.id}/candidacies/${CANDIDACY_ID}/feasibility-demat-autonome/send-file-certification-authority/`;
...
const createFeasibilitySendFileCertificationAuthorityHandlers = (
    ...
    fvae.query(
      ...
      graphQLResolver({
        getCandidacyById: {
          ...candidacy,
          certificationAuthorities:
            feasibility.candidacy.certificationAuthorities ??
...
test.describe("AAP send file to certification authority page", () => {
  const goTo = async (page: import("@playwright/test").Page) => {
    ...
    await waitGraphQL(page, QUERY_NAME);
  };
  ...
  test.describe("single certification authority, ready to be sent", () => {
    ...
    test.use({
      mswHandlers: [
        [
          ...createFeasibilitySendFileCertificationAuthorityHandlers(
            feasibility,
          ),
        ],
        { scope: "test" },
