...
const perimetreAccompagnementQuery = graphql(`
  query getOrganismForPerimetreAccompagnementForm($organismId: ID!) {
...
export const usePerimetreAccompagnementForm = ({
  organismId,
}: {
  organismId: string;
}) => {
  ...
  const {
    ...
    queryFn: () =>
      graphqlClient.request(perimetreAccompagnementQuery, { organismId }),
  });
