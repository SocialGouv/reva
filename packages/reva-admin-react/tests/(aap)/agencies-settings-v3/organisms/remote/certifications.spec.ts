...
const PAGE_URL = `/admin2/agencies-settings-v3/${MAISON_MERE_ID}/organisms/${ORGANISM_ID}/remote/perimetre-accompagnement/certifications/`;
const PERIMETRE_PAGE_URL = `/admin2/agencies-settings-v3/${MAISON_MERE_ID}/organisms/${ORGANISM_ID}/remote/perimetre-accompagnement/`;
...
test.describe("navigation", () => {
  ...
  test("navigates back to perimetre accompagnement page via Retour", async ({
    ...
    await expect(page).toHaveURL(PERIMETRE_PAGE_URL);
  });
...
test.describe("when form data fails to load", () => {
  ...
  test("displays load error alert", async ({ page }) => {
    await login({ role: "aap", page });
    await page.goto(PAGE_URL);
    await waitGraphQL(page, "getOrganismForOrganismRemotePage");
