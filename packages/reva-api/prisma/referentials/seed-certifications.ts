...
export async function seedCertificationsIfNone(prisma: PrismaClient) {
  const certificationsCount = await prisma.certification.count();

  if (certificationsCount === 0) {

    await upsertCertificationsFromCsv(prisma);

    await updateCerificationsInfo(prisma);

    await disableCertificationsNotInRncpIdList(prisma, [
      "17163",
      "5983",
      "34690",
      "34691",
...
async function disableCertificationsNotInRncpIdList(prisma: PrismaClient, availableCertifications: string[]) {
...
async function updateCerificationsInfo(prisma: PrismaClient) {
...
async function upsertCertificationsFromCsv(prisma: PrismaClient) {
  const inserts = new Promise((resolve, reject) => {
    ...
    fs.createReadStream(path.resolve(__dirname, "data-seed.csv"))
      ...
      .on(
        "data",
        (row) => (promiseChain = promiseChain.then(async () => {
