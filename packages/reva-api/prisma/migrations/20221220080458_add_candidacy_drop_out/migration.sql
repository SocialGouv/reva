-- AlterTable
ALTER TABLE "candidacy" ADD COLUMN     "candidacy_drop_out_id" UUID;

-- CreateTable
CREATE TABLE "drop_out_reason" (
    "id" UUID NOT NULL DEFAULT uuid_generate_v4(),
    "label" VARCHAR(255) NOT NULL,
    "created_at" TIMESTAMPTZ(6) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updated_at" TIMESTAMPTZ(6),

    CONSTRAINT "drop_out_reason_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "candidacy_drop_out" (
    "id" UUID NOT NULL DEFAULT uuid_generate_v4(),
    "drop_out_reason_id" UUID NOT NULL,
    "other_reason_content" TEXT,
    "status" "CandidacyStatus" NOT NULL DEFAULT E'PROJET',
    "drop_out_date" TIMESTAMP(3) NOT NULL,
    "created_at" TIMESTAMPTZ(6) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updated_at" TIMESTAMPTZ(6),

    CONSTRAINT "candidacy_drop_out_pkey" PRIMARY KEY ("id")
);

-- CreateIndex
CREATE UNIQUE INDEX "drop_out_reason_label_key" ON "drop_out_reason"("label");

-- AddForeignKey
ALTER TABLE "candidacy" ADD CONSTRAINT "candidacy_candidacy_drop_out_id_fkey" FOREIGN KEY ("candidacy_drop_out_id") REFERENCES "candidacy_drop_out"("id") ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "candidacy_drop_out" ADD CONSTRAINT "candidacy_drop_out_drop_out_reason_id_fkey" FOREIGN KEY ("drop_out_reason_id") REFERENCES "drop_out_reason"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
