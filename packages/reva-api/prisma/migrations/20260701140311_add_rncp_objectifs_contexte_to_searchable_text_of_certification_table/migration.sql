...
ADD COLUMN "searchable_text" tsvector GENERATED ALWAYS AS (
        to_tsvector(
            'simple',
            immutable_unaccent(
                (((COALESCE(label, ''::character varying))::text || ' '::text) || ((COALESCE(rncp_id, ''::character varying))::text || ' '::text) || ((COALESCE(rncp_type_diplome, ''::character varying))::text || ' '::text) || (COALESCE(rncp_objectifs_contexte, ''::text) || ' '::text))
            )
        )
    ) STORED;
