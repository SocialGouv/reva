import { Either, EitherAsync, identity, Left, Maybe, Right } from "purify-ts";
...
interface CreateAccountDeps {
    getAccountInIAM: (params: { email: string, username: string; }) => Promise<Either<string, Maybe<IAMAccount>>>;
...
export const createAccount = (deps: CreateAccountDeps) => async (params: {
    email: string,
    username: string;
    firstname?: string;
    lastname?: string;
    ...
    if (!params.email) {
        return Left(new FunctionalError(FunctionalCodeError.ACCOUNT_EMAIL_EMPTY, `Email invalide`));
    }
    ...
    if (params.group === 'admin') {
        ...
        return Left(new FunctionalError(FunctionalCodeError.ACCOUNT_ORGANISMID_EMPTY, `L'organismId est obligatoire pour un groupe "organism"`));
    } else {
        checkIfOrganismIsValid = 
            EitherAsync.fromPromise(() => deps.getOrganismById(params.organismId as string))
                .mapLeft(error => new FunctionalError(FunctionalCodeError.TECHNICAL_ERROR, error))
                .chain(async (maybeOrganism: Maybe<Organism>) => {
                    return maybeOrganism.toEither(new FunctionalError(FunctionalCodeError.ORGANISM_NOT_FOUND, `Organisme non trouvé`));
                });
    }

    const checkIfAccountInIAMAlreadyExists = 
        EitherAsync.fromPromise(() => deps.getAccountInIAM({ email: params.email, username: params.username }))
            .mapLeft(error => new FunctionalError(FunctionalCodeError.TECHNICAL_ERROR, error))
            .chain(async (maybeAccount: Maybe<IAMAccount>) => {
                if (maybeAccount.isJust()) {
                    throw new FunctionalError(FunctionalCodeError.ACCOUNT_ALREADY_EXISTS, `Un compte existe déjà pour cet email ou username`);
                }
    ...
    const createAccountWithProfile = (keycloakId: any) => EitherAsync.fromPromise(() => {
