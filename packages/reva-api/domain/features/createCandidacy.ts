...
interface CreateCandidacyDeps {
    createCandidacy: (params: { deviceId: string; certificationId: string; regionId: string; departmentId: string; }) => Promise<Either<string, Candidacy>>;
    getCandidacyFromDeviceId: (deviceId: string) => Promise<Either<string, Candidacy>>;
    notifyTeam: (candidacyId: string) => Promise<void>;
}

export const createCandidacy = (deps: CreateCandidacyDeps) => async (params: { deviceId: string; certificationId: string; regionId: string; departmentId: string; }): Promise<Either<FunctionalError, Candidacy>> => {
