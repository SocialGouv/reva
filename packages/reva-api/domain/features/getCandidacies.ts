...
interface GetCandidacySummariesDeps {
    ...
    getCandidacySummariesForUser: (IAMId: string) => Promise<Either<string, CandidacySummary[]>>;
}
...
export const getCandidacySummaries = (deps: GetCandidacySummariesDeps) => (params: {
    ...
    if (deps.hasRole("admin")) {
        return EitherAsync.fromPromise(() => deps.getCandidacySummaries())
        ...
        return EitherAsync.fromPromise(() => deps.getCandidacySummariesForUser(params.IAMId))
            .mapLeft(() => new FunctionalError(FunctionalCodeError.CANDIDACIES_NOT_FOUND, `Erreur lors de la récupération des aperçues candidatures`));
    } else {
