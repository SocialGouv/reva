...
export const confirmTrainingFormByCandidate = (deps: ConfirmTrainingFormByCandidateDeps) => (params: {
    candidacyId: string;
}) => {
    const existsCandidacyInValidation = EitherAsync.fromPromise(() => deps.existsCandidacyWithActiveStatus({
        ...
        if (!existsCandidacy) {
            return EitherAsync.liftEither(Left(`Le parcours candidat de la candidature ${params.candidacyId} ne peut être confirmé`));
        }
        return EitherAsync.liftEither(Right(existsCandidacy));
    })
        .mapLeft((error: string) => new FunctionalError(FunctionalCodeError.TRAINING_FORM_NOT_CONFIRMED, error));
    ...
    const updateCandidacy = EitherAsync.fromPromise(() => deps.updateCandidacyStatus({
        ...
        .mapLeft(() => new FunctionalError(FunctionalCodeError.TRAINING_FORM_NOT_CONFIRMED, `Erreur lors de la confirmation du parcours candidat de la candidature ${params.candidacyId}`));
