...
interface GetOrganismsDeps {
  getAAPOrganisms: (params: {
    candidacyId: string;
  }) => Promise<Either<string, Organism[]>>;
}

export const getAAPOrganismsForCandidacy =
  (deps: GetOrganismsDeps) => (params: { candidacyId: string }) => {
    const candidacies = EitherAsync.fromPromise(() =>
      deps.getAAPOrganisms(params)
    ).mapLeft(
      () =>
        new FunctionalError(
