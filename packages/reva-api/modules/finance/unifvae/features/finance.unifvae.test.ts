...
const enableRemoveFundingAndPaymentRequestStatusesFeature = async () =>
  createFeatureHelper({
    args: {
...
describe("Unifvae ayment request", () => {
  describe("DEMANDE_FINANCEMENT_ENVOYE and DEMANDE_PAIEMENT_ENVOYEE candidacy statues present", () => {
  ...
  describe("DEMANDE_FINANCEMENT_ENVOYE and DEMANDE_PAIEMENT_ENVOYEE candidacy statuses removed", () => {
    test("should fail to create paymentRequestUnifvae when candidacy was drop out less than 4 months ago then succeed after 4 months", async () => {
      await enableRemoveFundingAndPaymentRequestStatusesFeature();
