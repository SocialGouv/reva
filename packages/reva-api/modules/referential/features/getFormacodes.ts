import { readCsvRows } from "../../../prisma/seed/read-csv";
...
async function readGrandsDomains(): Promise<GrandDomaine[]> {
  const grandsDomainesData = await readCsvRows<{
    ...
    filePath:
      "../../modules/referential/features/getFormacodesGrandsDomaines.csv",
    headersDefinition: [
      "A",
      "B",
      "C",
...
async function readFormacodes(): Promise<Formacode[]> {
  const formacodesData = await readCsvRows<{
    ...
    filePath: "../../modules/referential/features/getFormacodes.csv",
