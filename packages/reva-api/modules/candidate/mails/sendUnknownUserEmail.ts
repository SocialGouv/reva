...
export const sendUnknownUserEmail = async (email: string) => {
  ...
  const htmlContent = `Vous avez demandé à recevoir un lien pour vous connecter mais votre e-mail n’existe pas dans notre base.
    ...
    <ul>
      ...
      <li>Si vous n’avez pas encore créé de compte, <a href="https://vae.gouv.fr/${isRevaCandidateActive ? "candidat/register" : "app"}/">vous devez d’abord vous inscrire</a>.</li>
    </ul>
    <br>
    L’équipe France VAE`;
