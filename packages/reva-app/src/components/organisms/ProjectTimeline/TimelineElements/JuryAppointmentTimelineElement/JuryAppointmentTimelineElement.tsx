...
export const JuryAppointmentTimelineElement = () => {
  const { state } = useMainMachineContext();

  const status = state.context.jury
    ? "active"
    : state.context.candidacyStatus === "DOSSIER_DE_VALIDATION_ENVOYE"
    ? "active"
    : "disabled";

  return (
    <TimelineElement title="Jury" status={status}>
      {({ status }) =>
